...
import com.JavaEmploymentTimes.Utils.ConverterUtil;
	...
	public String AddOffers(Offers off) {
		...
		try {
			PreparedStatement pre = con.prepareStatement(
					"insert into offer(couCode,couAmount,couStartDate,couEndDate,delInd,updateUser,updateDate,ipAddress)values(?,?,?,?,?,?,?,?)");
			pre.setString(1, off.getCouCode());
			pre.setFloat(2, off.getCouAmount());
			pre.setDate(3, ConverterUtil.convertFromJAVADateToSQLDate(off.getCouStartDate()));
			pre.setDate(4, ConverterUtil.convertFromJAVADateToSQLDate(off.getCouEndDate()));
			pre.setString(5, ("N"));
			pre.setString(6, off.getUpdateUser());
			pre.setDate(7, ConverterUtil.convertFromJAVADateToSQLDate(off.getUpdateDate()));
